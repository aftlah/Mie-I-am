...
export const authConfig = {
  trustHost: true, // <= WAJIB untuk production (fix UntrustedHost)

  // OPTIONAL tetapi direkomendasikan: baca URL dari .env
        ...
        if (stored.startsWith("scrypt:")) {
          ...
          if (salt && hex) {
            ...
            ok =
              ...
              timingSafeEqual(hashBuf, derived);
          }
  ...
  callbacks: {
    jwt: async ({ token, user }) => {
      ...
      const u =
        user as unknown as { role?: "admin" | "cashier" } | undefined;

      if (u?.role) {
        t.role = u.role;
      } else if (t.sub) {
        const existing = await db.staffUser.findUnique({
          where: { id: Number(t.sub) },
        });
